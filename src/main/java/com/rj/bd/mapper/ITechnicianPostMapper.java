...
public interface ITechnicianPostMapper extends BaseMapper<Technicianpost>{
	...
	@Select(value={"select u.user,t.technicianname,p.postcontent,p.posttime from Technicianpost p left join user u on p.userid=u.id left join technician t on p.technicianid=t.technicianid"})
	public List<Map<String, Object>> queryMoreAll();
}
